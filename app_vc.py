...
def load_models(args):
    ...
    # 根据设备类型确定是否使用fp16，CPU和MPS设备不支持某些half精度操作
    ...
    # Define mel function with MPS fallback
    if device.type == "mps":
        def to_mel(x):
            # Move input to CPU for STFT computation
            x_cpu = x.cpu()
            mel_spec = mel_spectrogram(x_cpu, **mel_fn_args)
