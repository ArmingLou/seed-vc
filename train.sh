...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "  --patience|-p   设置早停耐心值 (默认: 10)"
            echo "  --validation-interval|-v 设置验证间隔step (默认: 10)"
...
if [[ "$INTERACTIVE_MODE" = true ]]; then
    ...
    # 询问早停耐心值
    read -p "请输入早停耐心值 (默认: 10): " patience_input
    ...
    # 询问验证间隔
    read -p "请输入验证间隔step (默认: 10): " validation_interval_input
